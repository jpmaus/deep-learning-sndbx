    ...
    def generator(self, z, is_training=True, reuse=False):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            if self.z_dim == 128 :
                ...
                z_split = tf.split(z, num_or_size_splits=[split_dim] * 8 + [split_dim_remainder], axis=-1)

            else :
                split_dim = self.z_dim // 9
                split_dim_remainder = self.z_dim - (split_dim * 9)

                if split_dim_remainder == 0 :
                    z_split = tf.split(z, num_or_size_splits=[split_dim] * 9, axis=-1)
                else :
                    z_split = tf.split(z, num_or_size_splits=[split_dim] * 8 + [split_dim_remainder], axis=-1)
            ...
            x = resblock_up_condition(x, z_split[8], channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resup_01_1')
