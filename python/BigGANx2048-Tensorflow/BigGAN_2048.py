...
class BigGAN_2048(object):
    ...
    def generator(self, z, is_training=True, reuse=False):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            if self.z_dim == 128 :
                split_dim = 12
                split_dim_remainder = self.z_dim - (split_dim * 9)

                z_split = tf.split(z, num_or_size_splits=[split_dim] * 9 + [split_dim_remainder], axis=-1)

            else :
                split_dim = self.z_dim // 10
                split_dim_remainder = self.z_dim - (split_dim * 10)

                if split_dim_remainder == 0 :
                    z_split = tf.split(z, num_or_size_splits=[split_dim] * 10, axis=-1)
                else :
                    z_split = tf.split(z, num_or_size_splits=[split_dim] * 9 + [split_dim_remainder], axis=-1)
            ...
            x = resblock_up_condition(x, z_split[1], channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resup_16_0')
            x = resblock_up_condition(x, z_split[2], channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resup_16_1')
            ch = ch // 2

            x = resblock_up_condition(x, z_split[3], channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resup_08_0')
            x = resblock_up_condition(x, z_split[4], channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resup_08_1')
            ch = ch // 2

            x = resblock_up_condition(x, z_split[5], channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resup_04')

            # Non-Local Block
            x = self_attention_2(x, channels=ch, sn=self.sn, scope='self_attn')
            ch = ch // 2

            x = resblock_up_condition(x, z_split[6], channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resup_02_0')
            x = resblock_up_condition(x, z_split[7], channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resup_02_1')
            ch = ch // 2

            x = resblock_up_condition(x, z_split[8], channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resup_01_0')
            x = resblock_up_condition(x, z_split[9], channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resup_01_1')
    ...
    def discriminator(self, x, is_training=True, reuse=False):
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            ch = self.ch
            x = resblock_down(x, channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resdn_01_0')
            x = resblock_down(x, channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resdn_01_1')
            
            ch = ch * 2
            x = resblock_down(x, channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resdn_02_0')
            x = resblock_down(x, channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resdn_02_1')
            
            x = self_attention_2(x, channels=ch, sn=self.sn, scope='self_attn')
            
            ch = ch * 2
            x = resblock_down(x, channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resdn_04')
            
            ch = ch * 2
            x = resblock_down(x, channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resdn_08_0')
            x = resblock_down(x, channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resdn_08_1')
            
            ch = ch * 2
            x = resblock_down(x, channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resdn_16_0')
            x = resblock_down(x, channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resdn_16_1')
            
            x = resblock(x, channels=ch, use_bias=False, is_training=is_training, sn=self.sn, scope='resblk')
            x = relu(x)
